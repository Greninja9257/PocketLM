...
MIT for the code and the trained models in [`models/`](models/) — see
[`LICENSE`](LICENSE).

The corpus that trains the released models is generated by
`scripts/build_corpus.py` and carries no third-party licence. The `dev` branch
is different: it *downloads* corpora whose licences vary, several
non-commercial. Nothing downloaded is committed here (`data/` is gitignored),
but check [`dev/README.md`](dev/README.md) before shipping a model trained that
way.
